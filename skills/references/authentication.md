...
After approval, use one of the supported browser flows:

```text
multirunner connect --repo <owner/repo> --config <path>
multirunner connect --org <organization> --config <path>
```

The command writes `app_id`, `installation_id`, and a private key path while
removing `auth.pat`. It never prints generated credentials. Don't
...
repository runners, `admin:org` for organization runners, or
`manage_runners:enterprise` for enterprise runners. Don't require unrelated
...
and `repos`, performs bounded read-only checks for Actions availability and
workflow files using the configured multirunner credentials. Redact output
before sharing it. Validate private-key file existence and restrictive
